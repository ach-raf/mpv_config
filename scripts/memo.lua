...
function open_menu()
    ...
    local function is_valid_file(file)
        local invalid_extensions = {".jpg", ".txt", ".info"}
        for _, ext in ipairs(invalid_extensions) do
    ...
    local function populate_playlist()
        ...
        for i = 1, #playlist do
            ...
            if not is_valid_file(playlist_file) then
                -- Skip files with invalid extensions
                return
            end
        end
        -- Continue with the playlist population logic here
    end
    ...
    local function select_item(append)
        ...
        if append and item.value[1] == "loadfile" then
            local file_path = item.value[2]
            if is_valid_file(file_path) then
                populate_playlist()  -- Check file extensions during playlist population
